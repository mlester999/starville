...
export function createSupabaseAdminWorldGateway(client: SupabaseClient): AdminWorldGateway {
  return {
    ...
    getRevision: (identity, input) => execute(client, 'get_admin_world_revision', identity, input),
    compareRevisions: (identity, input) =>
      execute(client, 'compare_admin_world_revisions', identity, input),
    ...
    publishVersion: (identity, input) =>
      execute(client, 'publish_admin_world_revision', identity, input),
    reviewPublication: (identity, input) =>
      execute(client, 'review_admin_world_publication', identity, input),
    rollbackVersion: (identity, input) =>
      execute(client, 'rollback_admin_world_revision', identity, input),
